...
**If `happyyogis.com` also resolves to this site**, point it at `.in` with a
301 redirect rather than serving both. Two domains serving identical pages
splits the ranking signals between them, and the canonical tags now name `.in`,
so `.com` would be advertising a URL it does not own.
